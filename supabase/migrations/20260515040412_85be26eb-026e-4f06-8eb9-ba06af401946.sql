...
BEGIN
  ...
  END IF;
  IF p_payment_method NOT IN ('wallet','cash_on_delivery') THEN
  ...
  -- 1. Master order (payment_status starts as pending)
  ...
  LOOP
    ...
    -- ANTI-TAMPER: snapshot must match server price within 0.01
    ...
    -- TODO: [CONSTITUTIONAL DEBT] Wave P-9: Replace zero-UUID with actual vendor resolution.
  ...
  IF p_payment_method = 'wallet' THEN
    -- Resolve customer wallet (currency-agnostic: pick first active wallet)
    ...
    -- Manual atomic balance updates (no trigger reliance)
    ...
    UPDATE public.salsabil_master_orders
     ...
     WHERE id = v_master_id;
  END IF;
  -- cash_on_delivery: leave payment_status = 'pending', status = 'pending'
