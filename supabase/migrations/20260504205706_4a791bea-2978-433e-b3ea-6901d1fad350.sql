-- Phase 13.31 — Custom Items & FK Bypass
...
BEGIN
  ...
  IF v_payment_method = 'wallet'
     AND v_wallet_applied = 0
     AND (v_secondary_payment IS NULL OR v_secondary_payment = '') THEN
    RAISE EXCEPTION 'invalid_payment_configuration' USING ERRCODE = '22023';
  END IF;

  -- Address FK Bypass: if address doesn't exist, set NULL
  v_actual_address_id := NULL;
  IF v_address_id IS NOT NULL THEN
    SELECT id INTO v_actual_address_id FROM public.addresses WHERE id = v_address_id;
  END IF;
  ...
  INSERT INTO public.orders (
    ...
    v_user_id, v_total, v_payment_method, v_actual_address_id, v_notes, v_service_type, v_delivery_zone,
  ...
  FOR v_fulfillment IN SELECT * FROM jsonb_array_elements(v_fulfillments) LOOP
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(v_items) LOOP
      ...
      -- Product FK Bypass: if product doesn't exist in catalog, store as custom item with NULL product_id
      v_actual_product_id := NULL;
      IF v_product_id IS NOT NULL AND v_product_id <> '' THEN
        SELECT id INTO v_actual_product_id FROM public.products WHERE id = v_product_id;
      END IF;
      ...
      INSERT INTO public.order_items (
        ...
        v_order_id, v_fulfillment_id, v_actual_product_id,
        v_item->>'product_name',
      ...
      IF v_actual_product_id IS NOT NULL THEN
        UPDATE public.products
          SET stock = GREATEST(COALESCE(stock,0) - v_qty, 0)
          WHERE id = v_actual_product_id AND stock IS NOT NULL;
      END IF;
    END LOOP;
  END LOOP;
