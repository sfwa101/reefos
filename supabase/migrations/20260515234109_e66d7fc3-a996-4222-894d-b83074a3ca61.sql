-- WAVE C · Sovereign Veto & RPC Purification
-- Drop every known overload of process_checkout_sovereign to eradicate
-- PGRST203 ambiguity, then recreate a single canonical signature that
-- performs a deep veto inside the database itself.
...
DROP FUNCTION IF EXISTS public.process_checkout_sovereign(uuid, jsonb, jsonb, text);
DROP FUNCTION IF EXISTS public.process_checkout_sovereign(uuid, jsonb, jsonb, text, uuid);
DROP FUNCTION IF EXISTS public.process_checkout_sovereign(uuid, jsonb, jsonb, uuid, text);
DROP FUNCTION IF EXISTS public.process_checkout_sovereign(uuid, jsonb, jsonb, uuid, text, text);
DROP FUNCTION IF EXISTS public.process_checkout_sovereign(uuid, jsonb, jsonb, text, text, uuid);
...
DECLARE
  ...
  v_ledger_grand       numeric(14,2);
BEGIN
  -- ──────────────────── Sovereign Veto (Article 7.1) ────────────────────
  -- 1. The client-supplied hash MUST exist in the cashier_snapshots ledger.
  --    If not, the cart was tampered with, never previewed, or drifted.
  IF p_client_snapshot_hash IS NULL OR length(p_client_snapshot_hash) < 8 THEN
    RAISE EXCEPTION 'Sovereign Veto: missing client_snapshot_hash';
  END IF;

  SELECT snapshot_hash, output_payload
    INTO v_snapshot_row
    FROM public.cashier_snapshots
   WHERE snapshot_hash = p_client_snapshot_hash
   LIMIT 1;

  IF v_snapshot_row.snapshot_hash IS NULL THEN
    RAISE EXCEPTION 'Sovereign Veto: Invalid or compromised snapshot hash';
  END IF;

  v_ledger_grand := NULLIF(
    v_snapshot_row.output_payload #>> '{totals,grand_total}', ''
  )::numeric(14,2);

  IF v_ledger_grand IS NULL THEN
    RAISE EXCEPTION 'Sovereign Veto: snapshot output_payload missing totals.grand_total';
  END IF;
  ...
  -- ───────── Deep Veto: ledger totals MUST match computed totals ─────────
  -- The ledger's recorded grand_total (from the preview) must mathematically
  -- equal the grand_total just computed from authoritative DNA. Any drift
  -- (even by a fraction beyond rounding tolerance) is a constitutional
  -- breach and the entire transaction is aborted.
  IF abs(v_grand_total - v_ledger_grand) > 0.01 THEN
    RAISE EXCEPTION 'Sovereign Veto: Invalid or compromised snapshot hash (ledger=%, computed=%)',
      v_ledger_grand, v_grand_total;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.process_checkout_sovereign(uuid, jsonb, jsonb, uuid, text, text) TO authenticated;
