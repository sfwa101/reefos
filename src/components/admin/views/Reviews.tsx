...
import { UniversalAdminGrid, type BentoMetric, type Column } from "@/components/admin/UniversalAdminGrid";
...
export default function ReviewsAdmin() {
  const metrics: BentoMetric<ReviewRow>[] = [
    ...
    { key: "pos", label: "إيجابية (4-5)", icon: ThumbsUp, tone: "success",
      compute: (rows) => fmtNum(rows.filter((r) => (r.rating ?? 0) >= 4).length) },
    { key: "neg", label: "سلبية (1-2)", icon: ThumbsDown, tone: "accent",
      compute: (rows) => fmtNum(rows.filter((r) => (r.rating ?? 0) <= 2).length),
      urgent: (rows) => rows.some((r) => (r.rating ?? 0) <= 2) },
  ];

  const columns: Column<ReviewRow>[] = [
    { key: "body", className: "flex-1", render: (r) => (
      <>
