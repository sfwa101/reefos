...
## ملحق 2026-05-09 — تحقّق المراحل 55 → 58

> *"الإنسان واحد، المساحات متعدّدة. اليوم تحقّقت ثلاث مساحات تشغيلية،
> وتفعّلت الهوية السيادية ذات الستة أرقام."*

### 1. ثلاثة Workspaces تشغيلية بـ DNA موحّد
- **KDS Workspace** (المطبخ) — `_kds.tsx`
- **Dispatch Workspace** (التسليم الآمن) — `_dispatch.tsx`
- **Barq Workspace** (المندوب) — `_barq.tsx`

كلها RTL، Dark Shell، توكينز سيمانتية فقط (صفر ألوان مرمّزة يدوياً)،
متطابقة في الـ DNA البصري — تجسيد عملي للـ Adaptive Professional Runtime.

### 2. حلقة التسليم السيادية (OTP Sovereignty)
- إصدار OTP رباعي تلقائي عند `ready_for_pickup`.
- `confirm_handover` يرفض أي رمز غير مطابق (`invalid_otp`).
- العميل يرى رمزه في `OrderSuccess`، السائق يراه في `/driver-ops`.
- لا تسليم بلا برهان مشفّر.

### 3. الهوية السيادية تتجلّى — Tayseer Short ID
- كود الإحالة = آخر ٦ أرقام من الرقم القومي (أو مسكوك عشوائياً 100000–999999).
- "كل إنسان شريك نجاح. كل دعوة منفعة مشتركة."
- العمولات تُدفع من خزينة تيسير (محفظة `…0777`) بقيد محاسبي مزدوج صفري.
- مشاركة عبر واتساب بنقرة واحدة.

### 4. الـ Vehicle DNA — البذرة
- `profiles.vehicle_dna jsonb` جاهز لاستقبال خصائص المركبة (نمط،
  قدرات، نطاق كفاءة) — أساس مستقبلي للمطابقة الذكية بين المهمة
  والمركبة.

### 5. الدَّين الدوكتريني المتبقي → Phase 59
محفظة العميل (Tayseer) ما زالت تقرأ `wallet_transactions` (المرآة
القديمة) بدلاً من `ledger_entries` السيادية، وتحتوي على ٣١ تلوّثاً
لونياً يخالف القانون الرابع. **Phase 59 = إعادة تصميم سيادية كاملة
للمحفظة.**

> *الواجهة نتيجة. اليوم نتجت ثلاث واجهات تشغيلية من DNA واحد —
> غداً تنتج محفظة سيادية من نفس الجذر.*

---

## ملحق 2026-05-11 — المرحلة 60: البطاقة تولَد من القدرة

> *"المنتج لا يسأل: من أيّ قسم أنا؟ بل يقول: ما الذي أقدر عليه؟
> ومن قدراته تولَد بطاقته."*

### 1. مصدر واحد للحقيقة
- كل واجهات المتجر تقرأ من `usa_products` (192 صفّاً، 24 قسماً) عبر
  `catalogGateway`. الجدول القديم `salsabil_assets` تقاعد من مسار
  العرض.
- الأقسام الفارغة تعرض حالة فارغة حقيقية — لا بياضات، لا بيانات وهمية.

### 2. نظام البطاقات المُوجَّه بالقدرات
- مُحلِّل صافٍ في `CardTemplateRegistry.ts` يقرأ `capabilities[]` ويعيد
  اسم القالب: `meal → wholesale → subscription → health → configurable
  → standard`.
- لا توجد جملة `if (slug === "kitchen")` في أي مكان من مسار العرض —
  القانون الثالث (القدرة فوق الهوية) صار مُنفَّذاً تقنياً، لا مجرد بند.
- أوّل قالب مُسلَّم: `MealCard` — RTL، صورة مهيمنة 16:9، شرائح حصص
  حين `supports_variants`، شارة ETA، توكينز سيمانتية فقط.

### 3. التصنيفات تولَد من البيانات
- التصنيفات الفرعية لكل قسم تُشتق الآن من عمود `tags[]` (أعلى 12
  وسماً تكراراً) عبر `useSectionSubcategories`.
- اللحوم تعرض وسوم اللحوم. الحلويات تعرض وسوم الحلويات. لا قاموس
  مُتخشِّب يلوّث قسماً بقدرات قسم آخر.

### 4. ما تبقى
- خمسة قوالب أخرى (Standard / Wholesale / Subscription / Health /
  Configurable) على نفس عقد `CardTemplateProps`.
- بذرة `section_capabilities` للأقسام الـ 12 المتبقية.

> *في المرحلة 58 أنتجنا ثلاث واجهات تشغيلية من جذر واحد. اليوم في
> المرحلة 60 نُنتج كلّ بطاقات المتجر من جذر واحد: القدرة. الواجهة
> نتيجة. الجذر هو القانون.*
