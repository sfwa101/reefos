...
| 2026-05-12 | P-D/D-3 | 19 | 0 | removed | 0 | >0 | Bespoke route collapse (C4: 3 → 0). The 3 deferred bespoke pages (BasketsBuild, BasketsSubs, CompareHomeGoods) converted into declarative blocks (commerce.basket_builder, commerce.subscription_manager, commerce.compare_grid) served exclusively by store.$slug.tsx. All 17 hardcoded vertical routes are now eradicated. |
| 2026-05-12 | P-D/D-4 | 12 | 0 | removed | 0 | >0 | Catalog + Library + Driver + GroupBuy gateways landed. New `src/lib/{catalog,library,driver,group-buy}.functions.ts` (+ `*.queries.ts` for catalog/library/group-buy) expose `createServerFn` handlers behind `requireSupabaseAuth`: catalog (`listProductReviewsFn`, `listProductUnitsFn`), library (`getMyKycStatusFn`, `submitPrintJobFn`, `uploadPrintFileFn` via FormData), driver (`publishDriverPositionFn`, `getDriverPositionFn`, `resolveDriverIdFn`), group-buy (`listGroupBuyCampaignsFn` aggregator, `getMyGroupBuyPledgeFn`, `pledgeGroupBuyFn` RPC proxy). 6 UI files migrated off direct DB calls: `pages/ProductDetail.tsx`, `library/sections/SchoolLibrarySection.tsx`, `library/components/PrintWizard.tsx`, `driver/store/useDriverTelemetry.ts`, `driver/hooks/useActiveDriverTracking.ts` (channel EXEMPT), `group-buy/hooks/useGroupBuyEngine.ts` (channel EXEMPT). **C1: 19 → 12.** |
| 2026-05-12 | P-D/D-5 | 10 | 0 | removed | 0 | >0 | HR / Employee Gateway landed. New `src/lib/hr.functions.ts` + `src/lib/hr.queries.ts` expose `createServerFn` handlers behind `requireSupabaseAuth`: employee-facing (`getEmployeeHubFn`, `checkInFn`, `checkOutFn`, `submitAdvanceRequestFn`) and approver-facing (`listAdvanceRequestsFn`, `approveAdvanceFn`, `rejectAdvanceFn`) — the latter three gated by an inline `assertApproverRole` helper that calls `has_role(admin|finance|branch_manager)`. 2 UI files migrated off direct DB calls: `pages/EmployeeHub.tsx`, `pages/admin/AdvanceApprovals.tsx`. **C1: 12 → 10.** |
| 2026-05-12 | P-D/D-6 | 6 | 0 | removed | 0 | >0 | Admin Catalog Master-Data Gateway landed. New `src/lib/admin-catalog.functions.ts` + `src/lib/admin-catalog.queries.ts` expose `createServerFn` handlers behind `requireAdmin`: categories (`listCategoriesFn`, `upsertCategoryFn`, `deleteCategoryFn`), stores (`listStoresFn`, `upsertStoreFn`, `deleteStoreFn`), branches (`listBranchesFn`, `upsertBranchFn`, `setBranchActiveFn`, `deleteBranchFn`), suppliers (`listSuppliersFn`), and the atomic procurement RPC proxy (`submitPurchaseInvoiceFn` → `submit_purchase_invoice`). 4 admin UI files migrated off direct DB calls and `@/integrations/supabase/client`: `pages/admin/Categories.tsx`, `pages/admin/Stores.tsx`, `pages/admin/Branches.tsx`, `apps/reef-al-madina/features/admin/components/PurchaseInvoiceBuilder.tsx`. **C1: 10 → 6.** |
| 2026-05-12 | P-D/D-7+D-8 | 0 | 0 | removed | 0 | >0 | **Wave P-D COMPLETE.** Marketing, CRM, RBAC, and Ops admin gateways landed (`src/lib/{marketing,crm,rbac,ops}.functions.ts`, all `requireAdmin`). Final 6 admin UI files migrated off direct DB calls: `BannersPanel`, `CouponsPanel`, `FlashPanel` (marketing), `CustomerDetail` + `Notifications` (CRM), `RolePermissions` (RBAC), and `BusinessOpsDashboard` (ops — three `useQuery` calls + visibility-aware realtime channel collapsed into a single `getOpsKpisFn` aggregator polled every 15s). C1 is verifiably **ZERO** — only a single comment string referencing `supabase.from(...)` remains in `useCartOrchestrator.ts`. The UI layer is fully decoupled from the database. |
| 2026-05-12 | P-E/E-1+E-2 | 0 | 0 | removed | 0 | 0 | Wave P-E kickoff. 8 lib data modules (`butcheryPrep`, `sweetsFulfillment`, `kitchenMenu`, `restaurants`, `baskets`, `savedBaskets`, `library`, `villageMeta`) renamed to capability-driven kebab-case (`weighed-prep-rules`, `custom-fulfillment-rules`, `meal-menu-config`, `vendor-menu-config`, `bundle-thresholds`, `saved-bundles`, `digital-borrowing`, `sourcing-meta`). 3 pricing strategies renamed in lockstep: `MeatPricingStrategy → WeighedPricingStrategy`, `SweetsPricingStrategy → CustomFulfillmentPricingStrategy`, `WholesalePricingStrategy → BulkTierPricingStrategy` (plus their `*Selection` types). 44 import sites rewritten across `src/`. Build green. |
| 2026-05-12 | P-E/E-3 to E-7 | 0 | 0 | removed | 0 | 0 | **Wave P-E COMPLETE.** 8 feature folders renamed to capability domains under `src/apps/reef-al-madina/features/`: `meat → weighed-prep`, `sweets → custom-fulfillment`, `pharmacy → health-care`, `baskets → commerce-bundles`, `library → digital-borrowing`, `recipes → instruction-guides`, `restaurants → vendor-menu`, `subscriptions → recurring-orders` (no `wholesale` folder existed). All `@/apps/reef-al-madina/features/*` import sites rewritten. All C5 violations (Article 3a) eradicated from file paths and code identifiers. **The Salsabil OS UI layer is now completely generic, vertical-agnostic, and Sovereign. Purification COMPLETE.** |
